// Drill/ops task: mint CinaCreditV2 with the dedicated minter identity.
// Mirrors the chain-worker settlement
// mint so the circular-economy drill can run end-to-end from CI.
//
// Usage (env): CINATOKEN_MINTER_PRIVATE_KEY + CREDIT_MINT_TO + CREDIT_MINT_AMOUNT_ETH
...
const PK = requirePrivateKey(
  process.env.CINATOKEN_MINTER_PRIVATE_KEY,
...
if (!/^0x[0-9a-fA-F]{40}$/.test(TO))
  throw new Error("CREDIT_MINT_TO must be a 0x… address")

function resolveV2() {
  if (process.env.CINACREDIT_V2_ADDRESS)
    return process.env.CINACREDIT_V2_ADDRESS
  ...
  throw new Error(
    "CinaCreditV2 address unknown — set CINACREDIT_V2_ADDRESS or restore the receipt artifact"
  )
}
...
if (receipt.status === "reverted")
  throw new Error(`mintTo reverted (tx ${hash})`)
